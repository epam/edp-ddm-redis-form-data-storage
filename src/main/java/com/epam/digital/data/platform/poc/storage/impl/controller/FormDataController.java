...
public class FormDataController {
    ...
    @PostMapping("/form")
    public void createForm(@RequestHeader(KEY) String key,
                     @RequestBody FormDataDto formDataDto) {
        repository.putFormData(key, formDataDto);
    }

    @DeleteMapping("/form")
    public void deleteForms(@RequestHeader("prefix") String keyPrefix) {
        var keys = repository.getKeys(keyPrefix);
        repository.delete(keys);
    }

    @GetMapping("/form")
    public Optional<FormDataDto> getFormData(@RequestHeader(KEY) String key) {
